...
export class GenerateExcelService {
  ...
  private createErrorCheckSheet(data: { rejz: any[], pzn: pznObject[], wnpz: wnpzObject[], mapz: mapzObject[] }): XLSX.WorkSheet {
    ...
    // Ekstrakcja kodów transakcji z PZN, WNPZ i MAPZ
    ...
    const errorCheckData = [
      ...
      [''], 
      ['Unikalne Kody Transakcji', 'Suma dok dost (PZN)', 'Suma PZAmount (WNPZ)', 'Suma PZAmount (MAPZ)']
    ];

    // Dodanie sum dla każdej transakcji
    Array.from(transactionCodes).forEach(code => {
      ...
      errorCheckData.push([code, pznSum.toString(), wnpzSum.toString(), mapzSum.toString()]);
    });
