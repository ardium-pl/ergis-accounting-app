...
export class MergerService {
  ...
  readonly isNegativeDataValid = computed(
    () => this._negativesDataJson()?.length > 0
  );
  ...
  private _processData(
    ...
    if (positives.length == 0) {
      const negativeExchangeRate =
        negativeObject.kwotaWZł / negativeObject.kwotaWWalucie;
      negatives = this._retrieveUnusedNegatives(
        negativeAmount,
        negatives,
    ...
    while (
      (positives.length > 0 || !isNaN(positiveAmount)) &&
      negatives.length > 0
    ) {
      const negativeExchangeRate =
        negativeObject.kwotaWZł / negativeObject.kwotaWWalucie;
      const positiveExchangeRate =
        positiveObject.kwotaWZł / positiveObject.kwotaWWalucie;
      ...
      const currencyCorrection =
        correctionAmount! * (negativeExchangeRate - positiveExchangeRate);
  ...
  private _retrieveUnusedNegatives(
    negativeAmount: number,
    negativeArray: MergerObject[],
