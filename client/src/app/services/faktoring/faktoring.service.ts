...
import {
  ...
  FaktoringMode,
  FaktoringObject,
  FinalFaktoringObject,
  LeftOverObject,
  LeftoversFlag,
} from './faktoring.types';
...
export class FaktoringService {
  ...
  public setCsvFile(file: File): boolean {
    ...
    reader.onload = e => {
      ...
      try {
        ...
        const faktoringObjectsAndCorrections = csvObjects.map(this._mapRawCsvObject);
        const faktoringObjects: FaktoringObject[] = [];

        for (let i = 0; i < faktoringObjectsAndCorrections.length; i++) {
          const obj = faktoringObjectsAndCorrections[i];
          if (obj.korekta) {
            faktoringObjects.at(-1)!.corrections.push(obj);
            continue;
          }
          faktoringObjects.push(obj);
        }

        this._csvArray.set(faktoringObjects);
        console.log(faktoringObjects);
  ...
  private _mapRawCsvObject(rawObject: CsvObject<keyof FaktoringObject>): FaktoringObject {
    ...
    return {
      ...
      korekta: rawObject.korekta === 'true',
      konto: rawObject.konto ?? '',
      subkonto: rawObject.subkonto ?? '',
      mpk: rawObject.mpk ?? '',
